...
const getPartsBasedOnBreakpoint = () => {
  ...
  if (isSmallScreen) {
    return 1; // Divide array into 1 part
  } else if (isMediumScreen) {
    return 3; // Divide array into 3 parts
  } else if (isLargeScreen) {
    return 5; // Divide array into 5 parts
  }
};
...
const createGallery = async () => {
  ...
  const parts = divideArray(bookList);
  gallery.innerHTML = "";

  for (let part of parts) {
    const column = document.createElement("div");
    column.className = "space-y-4";
    for (const book of part) {
      const imageContainer = document.createElement("div");
      imageContainer.className =
        "rounded-lg bg-white overflow-hidden shadow-lg shadow-slate-300 relative group";
      ...
      title.className = "text-sm text-black text-bolder text-center py-2";
      ...
      imageContainer.append(image, title, id, overlay);
